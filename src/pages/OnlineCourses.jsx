...
export default function OnlineCourses() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetchCourses(),
      supabase.auth.getSession().then((result) => result?.data?.session?.user?.id || null),
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.length === 0 ? (
            <div className="col-span-full text-center py-16">
              <p className="text-black/70 dark:text-gray-300 text-lg">No courses available yet.</p>
            </div>
          ) : (
            courses.map((course) => {
                ...
                <Link key={course.id} to={`/course/${course.id}`} className="glass-card rounded-2xl overflow-hidden hover:shadow-lg transition-shadow group">
                  {course.image && (
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-[10px] font-medium text-teal-dark dark:text-teal-light bg-teal/15 dark:bg-teal-dark/20 px-2 py-0.5 rounded-full">{course.category}</span>
                      ...
                      {course.video_url && <span className="text-[10px] font-medium text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 rounded-full">🎬 Video</span>}
                      {course.pdf_url && <span className="text-[10px] font-medium text-sky-600 dark:text-sky-400 bg-sky-100 dark:bg-sky-900/30 px-2 py-0.5 rounded-full">📄 PDF</span>}
                    </div>
                    <h3 className="font-semibold text-black dark:text-gray-100">{course.title}</h3>
                    <p className="text-sm text-black/60 dark:text-gray-400 mt-1 line-clamp-2">{course.description}</p>
                    <div className="flex items-center justify-between mt-3">
                      {course.price > 0 ? (
                        <span className="text-sm font-bold text-black dark:text-gray-100">${course.price}</span>
                      ) : (
