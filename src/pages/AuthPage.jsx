...
export default function AuthPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (mode === 'signup') {
      ...
      if (!surname || !name || !phone) { setError('Fill in all fields.'); return; }

      const { data, error: signUpErr } = await supabase.auth.signUp({ email, password });
      if (signUpErr) { setError(signUpErr.message); return; }
      if (!data.user) { setError('Sign up failed. Try again.'); return; }

      await supabase.from('profiles').insert({ id: data.user.id, surname, name, phone, email });
      navigate('/templates');
    } else {
        ...
        <div className="slide-right w-full max-w-md">
          <div className="glass-card rounded-3xl p-8 sm:p-10">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-blue-900 dark:text-gray-100"><span className="text-blue-900 dark:text-gray-100">Inno</span><span className="text-teal">veta</span></h2>
              <p className="text-sm text-warm-gray dark:text-gray-400 mt-1">{mode === 'signin' ? 'Sign in to continue' : 'Create your account'}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {mode === 'signup' && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-warm-gray dark:text-gray-400 mb-1">Surname</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-warm-gray dark:text-gray-400 mb-1">Name</label>
              ...
              <div>
                <label className="block text-xs font-medium text-warm-gray dark:text-gray-400 mb-1">Email</label>
              ...
              {mode === 'signup' && (
                <div>
                  <label className="block text-xs font-medium text-warm-gray dark:text-gray-400 mb-1">Phone number</label>
              ...
              <div>
                <label className="block text-xs font-medium text-warm-gray dark:text-gray-400 mb-1">Password</label>
              ...
              <button type="submit" className="w-full px-6 py-3.5 bg-gradient-to-r from-teal to-teal-dark text-white text-sm font-semibold rounded-xl hover:from-teal-dark hover:to-teal transition-all shadow-sm cursor-pointer">
                {mode === 'signin' ? 'Sign In' : 'Create Account'}
              </button>
            </form>

            <p className="text-center text-xs text-warm-gray dark:text-gray-400 mt-5">
              {mode === 'signin' ? (
