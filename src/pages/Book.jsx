...
export default function Book() {
  ...
  const serviceType = searchParams.get('type') || null;
  ...
  async function handleSubmit(e) {
    ...
    if (domainParam) {
      ...
      message = fd.get('message')?.trim() || `Service request: ${serviceParam} (${serviceType || ''})`;
    } else {
    ...
    const booking = {
      ...
      email: fd.get('email'),
      phone: fd.get('phone') || '',
      type: domainParam ? 'Domain' : serviceParam ? (serviceType || 'Service') : (template?.category || ''),
      message,
      ...
      <div className="max-w-6xl mx-auto px-5 py-16">
        <div className="text-center mb-10">
          <span className="inline-block text-xs font-medium text-teal-dark dark:text-teal-light bg-white/70 dark:bg-black/70 backdrop-blur-md px-4 py-1.5 rounded-full mb-3 shadow-sm border border-white/40 dark:border-white/10">
            {domainParam ? '✦ Register Domain' : serviceParam ? '✦ Book Service' : '✦ Hire Me'}
          </span>
          <h2 className="text-3xl sm:text-4xl font-bold text-warm-dark dark:text-gray-100">
            {domainParam ? 'Claim Your Domain' : serviceParam ? 'Book This Service' : "Let's Work Together"}
          </h2>
          <p className="text-black dark:text-gray-300 mt-2 max-w-lg mx-auto">
            {domainParam
              ...
              : serviceParam
                ? `Request ${serviceParam} — fill out the form and we'll get back to you.`
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-2 glass-card rounded-2xl p-6 sm:p-8 space-y-5">
            {domainParam ? (
              ...
              <div className="glass-card rounded-xl p-4 flex items-center gap-4 bg-gradient-to-r from-teal/5 to-teal-dark/5 dark:from-teal/10 dark:to-teal-dark/10 border border-teal/20 dark:border-teal-dark/20">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-teal to-teal-dark flex items-center justify-center text-white text-lg font-bold shrink-0">📋</div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-black dark:text-gray-100">{serviceParam}</p>
                  <p className="text-xs text-black/60 dark:text-gray-400 capitalize">{serviceType?.replace(/-/g, ' ') || 'Service'}</p>
                </div>
                <span className="text-[10px] font-medium text-teal-dark dark:text-teal-light bg-teal/15 dark:bg-teal-dark/20 px-2 py-1 rounded-full shrink-0">Service</span>
              </div>
            ) : (
            ...
            <button
              type="submit"
              disabled={sending || (!templateId && !domainParam && !serviceParam) || (domainParam && !domainPrice)}
